import { useState } from 'react'
import { accountUser } from '../../data/accountPageData'
import {
  hasValidationErrors,
  validateAccountForm,
  validateEmail,
  validateName,
  validatePhone,
} from '../../utils/accountValidation'
import { CameraIcon, ShieldCheckIcon, ArrowRightIcon, CloseIcon } from '../Icons'

function FormField({
  label,
  id,
  value,
  onChange,
  onBlur,
  type = 'text',
  className = '',
  error,
  hint,
  maxLength,
}) {
  return (
    <div className={className}>
      <label htmlFor={id} className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-500">
        {label}
      </label>
      <input
        id={id}
        type={type}
        value={value}
        onChange={onChange}
        onBlur={onBlur}
        maxLength={maxLength}
        aria-invalid={Boolean(error)}
        aria-describedby={error ? `${id}-error` : hint ? `${id}-hint` : undefined}
        className={`mt-2 w-full rounded-2xl bg-[#EDE6DC] px-4 py-3 text-sm font-medium text-gray-800 outline-none focus:ring-2 ${
          error ? 'ring-2 ring-[#D15151]/40' : 'focus:ring-[#D15151]/30'
        }`}
      />
      {hint && !error ? (
        <p id={`${id}-hint`} className="mt-1.5 text-xs text-gray-500">
          {hint}
        </p>
      ) : null}
      {error ? (
        <p id={`${id}-error`} className="mt-1.5 text-xs font-semibold text-[#D15151]">
          {error}
        </p>
      ) : null}
    </div>
  )
}

function ValidationPopup({ errors, onClose }) {
  const messages = Object.values(errors).filter(Boolean)

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4"
      role="dialog"
      aria-modal="true"
      aria-labelledby="validation-popup-title"
    >
      <div className="w-full max-w-md rounded-[1.75rem] bg-white p-6 shadow-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 id="validation-popup-title" className="text-lg font-extrabold text-[#3D2C2C]">
              Please fix the errors
            </h3>
            <p className="mt-1 text-sm text-gray-500">
              Some fields need attention before your changes can be saved.
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
            className="rounded-full border-0 bg-transparent p-1 text-gray-500 hover:bg-gray-100 hover:text-gray-800"
          >
            <CloseIcon className="h-5 w-5" />
          </button>
        </div>

        <ul className="mt-4 space-y-2">
          {messages.map((message) => (
            <li
              key={message}
              className="rounded-xl bg-[#FDE8E8] px-3 py-2 text-sm font-medium text-[#8B2E2E]"
            >
              {message}
            </li>
          ))}
        </ul>

        <button
          type="button"
          onClick={onClose}
          className="btn-zoom-hover mt-5 w-full rounded-full bg-[#D15151] px-6 py-3 text-sm font-bold text-white hover:bg-[#b84242]"
        >
          OK
        </button>
      </div>
    </div>
  )
}

const emptyErrors = {
  fullName: '',
  email: '',
  phone: '',
  preferredName: '',
}

export default function AccountSettingsForm() {
  const [form, setForm] = useState(accountUser)
  const [errors, setErrors] = useState(emptyErrors)
  const [showPopup, setShowPopup] = useState(false)
  const [saveMessage, setSaveMessage] = useState('')

  const updateField = (field) => (e) => {
    setSaveMessage('')
    setForm((prev) => ({ ...prev, [field]: e.target.value }))
    if (errors[field]) {
      setErrors((prev) => ({ ...prev, [field]: '' }))
    }
  }

  const validateField = (field, value) => {
    switch (field) {
      case 'fullName':
        return validateName(value, 'Full name')
      case 'preferredName':
        return validateName(value, 'Preferred name')
      case 'email':
        return validateEmail(value)
      case 'phone':
        return validatePhone(value)
      default:
        return ''
    }
  }

  const handleBlur = (field) => () => {
    const message = validateField(field, form[field])
    setErrors((prev) => ({ ...prev, [field]: message }))
  }

  const handleSave = () => {
    const nextErrors = validateAccountForm(form)
    setErrors(nextErrors)

    if (hasValidationErrors(nextErrors)) {
      setShowPopup(true)
      setSaveMessage('')
      return
    }

    setShowPopup(false)
    setSaveMessage('Your account details were saved successfully.')
  }

  return (
    <>
      {showPopup && hasValidationErrors(errors) ? (
        <ValidationPopup errors={errors} onClose={() => setShowPopup(false)} />
      ) : null}

      <article className="relative overflow-hidden rounded-[2rem] bg-white p-6 shadow-soft sm:p-8 lg:p-10">
        <span className="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-[#D4F5EF]/60 blur-sm" />
        <span className="pointer-events-none absolute right-12 top-8 h-24 w-24 rounded-full bg-[#E0F2F1]/80 blur-md" />

        {saveMessage ? (
          <div className="relative z-10 mb-6 rounded-2xl bg-[#D4F5EF] px-4 py-3 text-sm font-semibold text-[#2D6A64]">
            {saveMessage}
          </div>
        ) : null}

        <div className="relative z-10 flex flex-col gap-6 border-b border-gray-100 pb-8 sm:flex-row sm:items-center">
          <div className="relative shrink-0">
            <img
              src={form.profileImage}
              alt="Profile"
              className="h-24 w-24 rounded-2xl object-cover sm:h-28 sm:w-28"
            />
            <button
              type="button"
              aria-label="Change profile photo"
              className="absolute -bottom-1 -right-1 flex h-8 w-8 items-center justify-center rounded-full bg-[#D15151] text-white shadow-md"
            >
              <CameraIcon className="h-4 w-4" />
            </button>
          </div>

          <div className="flex-1">
            <p className="font-bold text-gray-900">Profile Photo</p>
            <p className="mt-1 text-sm text-gray-500">Recommended size: 400×400px. JPG or PNG.</p>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <button
                type="button"
                className="rounded-full bg-[#F0EBE4] px-5 py-2 text-sm font-semibold text-gray-700 transition hover:bg-[#e5ddd4]"
              >
                Change
              </button>
              <button type="button" className="text-sm font-semibold text-[#D15151] hover:underline">
                Remove
              </button>
            </div>
          </div>
        </div>

        <div className="relative z-10 mt-8 grid gap-5 sm:grid-cols-2">
          <FormField
            label="Full Name"
            id="fullName"
            value={form.fullName}
            onChange={updateField('fullName')}
            onBlur={handleBlur('fullName')}
            error={errors.fullName}
            maxLength={40}
            hint="Letters only, up to 30 letters."
          />
          <FormField
            label="Email Address"
            id="email"
            type="email"
            value={form.email}
            onChange={updateField('email')}
            onBlur={handleBlur('email')}
            error={errors.email}
            hint="Must include @ and .com (e.g. name@example.com)."
          />
          <FormField
            label="Phone Number"
            id="phone"
            value={form.phone}
            onChange={updateField('phone')}
            onBlur={handleBlur('phone')}
            error={errors.phone}
            hint="Format: +[country code] [10-digit number], e.g. +1 5551234567."
          />
          <FormField
            label="Preferred Name"
            id="preferredName"
            value={form.preferredName}
            onChange={updateField('preferredName')}
            onBlur={handleBlur('preferredName')}
            error={errors.preferredName}
            maxLength={40}
            hint="Letters only, up to 30 letters."
          />
          <div className="sm:col-span-2">
            <label htmlFor="bio" className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-500">
              Bio
            </label>
            <textarea
              id="bio"
              rows={4}
              value={form.bio}
              onChange={updateField('bio')}
              className="mt-2 w-full resize-none rounded-2xl bg-[#EDE6DC] px-4 py-3 text-sm font-medium leading-relaxed text-gray-800 outline-none focus:ring-2 focus:ring-[#D15151]/30"
            />
          </div>
        </div>

        <div className="relative z-10 mt-8 flex flex-col gap-4 border-t border-gray-100 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="flex items-center gap-2 text-sm text-[#2D6A64]">
            <ShieldCheckIcon className="h-5 w-5 shrink-0" />
            Your data is securely stored and never shared.
          </p>
          <button
            type="button"
            onClick={handleSave}
            className="btn-zoom-hover inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#D15151] to-[#E56B6B] px-8 py-3.5 text-sm font-bold text-white hover:from-[#b84242] hover:to-[#d55a5a]"
          >
            Save Changes
            <ArrowRightIcon className="h-4 w-4" />
          </button>
        </div>
      </article>
    </>
  )
}
