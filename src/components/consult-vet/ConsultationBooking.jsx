...
function StepBadge({ number }) {
  return (
    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#52CFC2] text-sm font-bold text-white">
      {number}
    </span>
  )
}
    ...
    <section className="px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
        <div className="space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-start rounded-2xl p-5 text-left transition ${
                      selected
                        ? 'bg-[#52CFC2] text-white shadow-md'
                        : 'bg-white text-gray-900 hover:bg-white/80'
                    }`}
                  >
                    <Icon className={`h-6 w-6 ${selected ? 'text-white' : 'text-[#52CFC2]'}`} />
          ...
          <article className="flex items-center justify-between gap-4 rounded-3xl bg-[#52CFC2] p-6 shadow-soft">
            <div>
        ...
        <div className="space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center rounded-2xl px-3 py-4 text-center transition ${
                      selected
                        ? 'bg-[#52CFC2] text-white'
                        : 'bg-white text-gray-800 hover:bg-white/80'
                    }`}
                  >
                    <Icon className={`h-5 w-5 ${selected ? 'text-white' : 'text-[#52CFC2]'}`} />
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-full px-5 py-3 text-sm font-semibold transition ${
                      selected
                        ? 'border-2 border-[#52CFC2] bg-white text-gray-900'
                        : 'bg-white text-gray-700 hover:bg-white/80'
                    }`}
                  >
                    <span>{slot.label}</span>
                    {slot.badge && (
                      <span className="rounded-full bg-[#52CFC2] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">
                        {slot.badge}
                      </span>
                    )}
                  </button>
          ...
          <article className="flex flex-wrap items-center justify-between gap-4 rounded-3xl bg-[#E1D8D2] p-6 shadow-soft">
            <div>
            ...
            <button
              type="button"
              className="btn-zoom-hover inline-flex items-center gap-2 rounded-full bg-[#64E0D2] px-6 py-3.5 text-sm font-bold text-white hover:bg-[#64E0D2]"
            >
              Start consultation
