...
export default function WellnessPlan() {
  return (
    <section className="px-4 py-6 sm:px-6 lg:px-8">
      <article className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-gradient-to-br from-[#CFE1E0] via-white to-white p-8 shadow-soft sm:p-10">
        <div className="grid gap-10 lg:grid-cols-2">
          <div>
            <div className="flex items-center gap-2 text-[#52CFC2]">
            ...
            <ul className="mt-6 space-y-4">
              {wellnessAdvice.map((tip) => (
                <li key={tip} className="flex gap-3 text-sm leading-relaxed text-gray-600">
                  <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#52CFC2]" />
                  {tip}
                </li>
              ))}
            </ul>
            <a
              href="#"
              className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#64E0D2] hover:underline"
            >
          ...
          <div className="rounded-3xl bg-[#E1D8D2] p-6">
              ...
              {suggestedProducts.map((product) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="mt-1 text-sm font-extrabold text-[#52CFC2]">{product.price}</p>
                  </div>
                </div>
              ))}
            </div>
